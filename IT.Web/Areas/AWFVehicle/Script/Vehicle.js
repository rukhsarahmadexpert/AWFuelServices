function Edit(Id) {
    $('#updateVehicle').show();
    $('#saveVehicle').hide();
    ajaxRequest("GET", "/AWFVehicle-Edit/" + Id, "", "json").then(function (result) {
...
$('#saveVehicle').click(function () {
    ...
    if (ValidVehicle()) {
        ...
        var Data = JSON.stringify({
            ...
            Model: $('#Model').val(),
            Color: $('#Color').val(),
            MulkiaExpiryDate: $('#MulkiaExpiry').val(),
            InsuranceExpiryDate: $('#InsuranceExpiry').val(),
            RegisteredRegion: $('#RegisteredRegion').val(),
            Brand: $('#Brand').val(),
            Comments: $('#Comments').val()
        })
        ajaxRequest("POST", "/AWFVehicle-Create", Data, "json").then(function (result) {
            if (result == "success");
            {
                sucessAdd();
...
$('#updateVehicle').click(function () {
    ...
    if (ValidVehicle()) {
        ...
        ajaxRequest("POST", "/AWFVehicle-Update", Data, "json").then(function (result) {
            if (result == "success");
            {
