...
$(document).ready(function () { 
    ...
    $('#mainrowgood #ExpenseType').change(function () {
        ...
        if ($(this).val().trim() == "Vehicle") {
            ...
            ajaxRequest("POST", "/LoadGeneralExpense", "", "json").then(function (result) {
...
$('#btnaddRow').click(function () {
    ...
    if (currentrow.find('.ExpenseType').val() == "Expense For") {
        isAllValid = false;
    ...
    if (currentrow.find('.ChoiceBox').val() == 0) {
        isAllValid = false;
        // swal("Let op!", "Voer inkoopprijs in", "warning");
        alert('Please select your desire choice');
        return;
    }

    else if ($("#mainrowgood #drpgoods").val() == "0") {
        isAllValid = false;
    ...
    else if ($("#mainrowgood .RowSubTotal ").val() == 0) {
        isAllValid = false;
        // swal("Let op!", "Voer inkoopprijs in", "warning");
        alert('Add Amount')
        return;
    ...
    if (isAllValid) {
        ...
        $('select').chosen();
        $('#mainrowgood .ExpenseType').val('Expense For');
    ...
    if ($('#ExpensePadNumber').val() != "") {
        ...
        if (IsValidationPass == true) {
            ...
            for (var i = 0; i < list.length; i++) {
                ...
                formData.append('expenseDetailsList[' + i + '][OnDates]', list[i].OnDates)
                    ...
                    success: function (result) {
                        if (result != "Failed") {
                            ...
                            if (SendEmail != null) {
                                ...
                                window.location.href = "/Expense-Details/" + result;
                            }
                        }
                        else {
                            alert(result);
        ...
        $('#ExpensePadNumber').css('border-color', 'red');
    }
}